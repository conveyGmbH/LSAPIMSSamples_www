...
async function handleShowAttachmentClick() {
  ...
  try {
    // Option 1: Si AttachmentIdList est disponible directement
    ...
    // Option 2: Recherche d'attachments via API
...
function handleTransferClick() {
  ...
  // Show CRM selection modal or redirect directly
  showCRMSelectionModal();
}

function showCRMSelectionModal() {
  // Create modal for CRM selection
  ...
  modal.innerHTML = `
    <div class="modal-content" style="max-width: 400px; text-align: center;">
      <div class="modal-header">
        <h3>Select CRM System</h3>
        <span class="close-modal" onclick="this.closest('.modal').remove()">&times;</span>
      </div>
      <div class="modal-body" style="padding: 20px;">
        <p>Choose which CRM system to transfer the lead to:</p>
        <div style="display: flex; flex-direction: column; gap: 15px; margin-top: 20px;">
          <button onclick="redirectToSalesforce()" class="btn-primary" style="padding: 15px 20px; border-radius: 8px; border: none; background: #0070d2; color: white; cursor: pointer; font-size: 16px;">
            <svg width="20" height="20" style="margin-right: 8px; vertical-align: middle;" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 6.62 5.367 11.987 11.988 11.987 6.62 0 11.987-5.367 11.987-11.987C24.004 5.367 18.637.001 12.017.001zM8.4 19.15V16.8h7.2v2.35H8.4zm7.2-4.7V12.1H8.4v2.35h7.2z"/>
            </svg>
            Salesforce CRM
          </button>
          <button onclick="redirectToDynamics()" class="btn-primary" style="padding: 15px 20px; border-radius: 8px; border: none; background: #0078d4; color: white; cursor: pointer; font-size: 16px;">
            <svg width="20" height="20" style="margin-right: 8px; vertical-align: middle;" viewBox="0 0 24 24" fill="currentColor">
              <path d="M24 12c0 6.627-5.373 12-12 12S0 18.627 0 12 5.373 0 12 0s12 5.373 12 12zM2.4 12c0 5.302 4.298 9.6 9.6 9.6s9.6-4.298 9.6-9.6S17.302 2.4 12 2.4 2.4 7.098 2.4 12z"/>
            </svg>
  ...
  // Add global functions for redirect
  window.redirectToSalesforce = () => {
    modal.remove();
    window.location.href = 'displayWceLeadTransfer.html';
  };
  
  window.redirectToDynamics = () => {
    modal.remove();
